...
export const getProducts = (keyword = '', currentPage = 1, price, category, rating = 0) => async(dis) => {
    try{
        
        dis({ type:ALL_PRODUCTS_REQUEST })
...
export const deleteProduct = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`http://localhost:4000/api/v1/admin/products/${id}`)
        dispatch({
